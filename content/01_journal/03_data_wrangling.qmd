...
toc()
#Print the top 10 companies with maximum patents
head(Count_Q1,10)
```
...
class(patent_tbl)
setDT(patent_tbl)
#Q2_patent_all has patent,patent data,assigned data all merged 
tic()
Q2_patent_all <- merge(x = patent_tbl, y = Q1_patent_data, 
                          ...
                          all.y = FALSE)
toc()
#Selecting a time frame (Aug 1 2014 to Aug 31 2014) and building a new data frame Q2_Aug_patents
Q2_Aug_patents<- with(Q2_patent_all, Q2_patent_all[(date >= "2014-08-01" & date <= "2014-08-31") ])
#Count_Aug has the data of all the organisations that got a patent in Aug 2014 in descending order 
...
#create data frame of top10 companies only to investigate the top 5 USPTO
...
#adding a new column with has Company name & Class together with "Company_Class" title
Q3_top10$Company_Class <- paste(Q3_top10$organization, Q3_top10$mainclass_id)
#Counting the Company_class and arranging in descending order
tic()
Q3_top10_CC<- Q3_top10 %>%
  count(Company_Class)%>% arrange(desc(n))
toc()

#displaying the most innovative class and its number of patents of that class
